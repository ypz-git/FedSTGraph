...
class TaskGen(BasicTaskGen):
    def __init__(self, dist_id=5, num_clients=325, rawdata_path='./benchmark/RAW_DATA/traffic', skewness = 0, seed = 0, local_hld_rate=0.0):
        super(TaskGen, self).__init__(benchmark='pems-bay_traffic_forecasting',
                                      dist_id=5,
                                      skewness=skewness,
    ...
    def load_data(self):
        ...
        if not os.path.exists(os.path.join(self.rawdata_path, 'data', 'traffic')):
            ...
            print('done')
        self.data_path = os.path.join(self.rawdata_path,'data','traffic','data','PEMS-BAY')
        self.adj_path = os.path.join(self.rawdata_path,'data','traffic','data','sensor_graph')
        self.adj_mx_name = 'adj_mx_bay.pkl'
